...
struct ProgressEvent : public QEvent
{
	...
	explicit ProgressEvent(bool saved_, const QString& message_)
		: QEvent(static_cast<Type>(EventId)), saved(saved_), message(message_)
	{}
...
void convertImages(QObject* receiver, volatile bool* stopped, const QStringList& sourceFiles,
	const QString& targetType)
{
	foreach(const QString& source, sourceFiles)
	{
		...
		QString message = saved ? QObject::tr("Saved %1").arg(QDir::toNativeSeparators(target)) 
			: QObject::tr("Failed to convert %1").arg(QDir::toNativeSeparators(source));

		QApplication::postEvent(receiver, new ProgressEvent(saved, message));
	}
}
#endif
...
void MainWindow::convertOrCancel()
{
	...
	{
		AQP::warning(this, tr("No Images Error"), tr("No matching file found"));
	}
	else
	{
		logEdit->clear();
...
void MainWindow::updateUi()
{
	if (stopped)
	{
		...
		convertOrCancelButton->setEnabled(!directoryEdit->text().isEmpty());
	}
	else
	{
...
void MainWindow::checkIfDone()
{
	...
	else 
	{
		...
		{
			message = tr("Converted %n/%1 image(s)", "", done).arg(total);
		}
...
{
	connect(directoryEdit, SIGNAL(textChanged(const QString&)), this, SLOT(updateUi()));
	connect(sourceTypeComboBox, SIGNAL(currentIndexChanged(const QString&)), 
...
void MainWindow::convertFiles(const QStringList& sourceFiles)
{
	...
	const QVector<int> sizes = AQP::chunkSizes(total, QThread::idealThreadCount());
